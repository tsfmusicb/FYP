...
- (void) setSoundFontAndMidFile:(NSString *)midiFile SoundFont:(NSString *)soundFont {
    _sfFile = soundFont;
    _midiFile = midiFile;
...
- (void) setupSampler:(UInt8) pn;
{
     ...
     */
    bankURL = [[NSURL alloc] initFileURLWithPath:_sfFile];
    //NSLog(@"set pn %d", pn);
...
- (void) loadMIDIFile
{
    ...
    NSURL *midiFileURL = [[NSURL alloc] initFileURLWithPath: _midiFile];
